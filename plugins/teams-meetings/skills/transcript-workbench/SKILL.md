...
input, and every transcript gets **Open** and **Reveal in Explorer** buttons that go through the OS.
...
Call `list_transcripts` and give the path, then offer `reveal_transcript` — opening Explorer with the
file selected answers the question far better than a path in prose does.
